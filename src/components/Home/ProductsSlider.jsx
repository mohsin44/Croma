...
const ProductSlider = ({ filter, sort }) => {
  const pojectid = import.meta.env.PROJECTID;
  const productsUrl =
  ...
  const [isNextArrowDisabled, setNextArrowDisabled] = useState(false);

  const handleBeforeChange = (oldIndex, newIndex) => {
    // Disable prev arrow initially and enable it after scrolling for the first time
    setPrevArrowDisabled(false);

    // Check if the next arrow should be disabled when reaching the end
  ...
  const [settings] = useState({
    ...
    prevArrow: <CustomPrevArrow disabled={isPrevArrowDisabled} />,
    nextArrow: <CustomNextArrow disabled={isNextArrowDisabled} />,
    responsive: [
      {
        breakpoint: 1280,
        settings: {
